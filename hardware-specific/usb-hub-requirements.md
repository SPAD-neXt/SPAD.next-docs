...
4. Rule of thumb is to have a psu that provides Devices\*0.5A + 0.5A , to kinda ensure the psu is not falling down if max is pulled of it. (Again cheap power adapters tend not to be able to provide their max output value constantly, especially right after power on). \
...
6. Also connecting 5 fip to **one** usb 2.0 hub will most likely fail as well, due to usb 2.0 bandwidth limitations. All devices connected to the hub **share** the bandwidth.
